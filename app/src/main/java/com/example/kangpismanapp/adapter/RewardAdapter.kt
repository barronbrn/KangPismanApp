...
class RewardAdapter : ListAdapter<Reward, RewardAdapter.ViewHolder>(DiffCallback()) {
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(reward: Reward) {
            ...
            points.text = "${reward.points} pts" // Tampilkan poin

            Glide.with(itemView.context)
                .load(reward.imageUrl)
                .placeholder(R.drawable.ic_reward) // Gambar placeholder
                .into(imageView)
        }
    }

    // 3. PERBAIKI DiffCallback
